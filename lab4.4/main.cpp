...
public:
    int text1;
    string bin;
    std::vector<std::bitset<8> > v;//временный битовый вектор 
    std::vector<std::bitset<1088> > x;//битовый вектор блока сообщений
    /********Преобразует строку, заданную в качестве входных данных, в вектор набора битов по 8 бит каждый*********/
    ...
    for(i = 0; i < str_len; i++){
        ...
        v.push_back(bits);
        bin=bin+bits.to_string<char,std::string::traits_type,std::string::allocator_type>();//создает одну большую строку во всех строках входного файла
        }
      }//двоичный преобразователь
    /******************Дополните строку сообщения до 1088 * k бит, используя правило заполнения 10 * 1*****/
    void Padding(std::vector<std::bitset<8> > b){
        int calc,check;
        int flag;
        flag=1;
        if(((b.size()*8)%1088)!=0)//проверьте, если уже кратно 1088
        {
            calc=1088-((b.size()*8)%1088);//количество бит для заполнения
            check=calc/8;
            while(check>0)
            {
                if(flag){
        ...
        else // if блоки сообщений уже кратны 8
        {
            check=1088/8;
            while(check>0)
            {
    ...
    /*******************Разделите блок по битрейту, то есть по 1088 бит каждый*************/
    void DivideBlocks(std::vector<std::bitset<8> > a)    {
        string temp;
        int amt,point;
        amt=(a.size()*8)/1088;//количество блоков, которые будут сгенерированы
        point=0;
        for(int i=0;i<amt;i++)
        {
            temp.clear();
            ...
            x.push_back(bits);//каждый блок сообщений из 1088 помещается в вектор
        }
     //   cout<<"Size of 1088 vector:"<<x.size()<<endl;
    }//разделить на блоки по 1088 бит
...
public:
    ...
    }// преобразует строку в 64-битный набор
    string buff,t1,t2;
    int k1,x;
    int arr[2][2];
    string* RC;
    ...
    void Sponge1(std::vector<std::bitset<1088> > c){
       ...
       // ****************************************************************
        ...
        k1=c.size();//количество блоков
        /********XOR каждый блок с губкой запуск функции Theta,Rho,Pi,Chi,Iota 24 раза (всего 24 * 5 шагов)****/
        for(int i=0;i<k1;i++){ 
            ...
            buff=t1+t2;//блок сообщения блокнота, чтобы сделать длину 1600 из 1088 по отступу 512 0
            std::bitset<1600>  bits(buff);
            sponge^=bits;//xor блок сообщений с губкой
            for(x=0;x<24;x++){
                Theta();
                Rho();
                Pi();
