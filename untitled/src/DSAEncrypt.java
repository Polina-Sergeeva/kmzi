...
public class DSAEncrypt extends  DSA{
    ...
    private void generateU() throws NoSuchAlgorithmException {
        BigInteger MD4FromS = new BigInteger(getMD5Hash(S.toString()), 16);
        BigInteger twoPowG = AlgorithmsDSA.pow(BigInteger.TWO, BigInteger.valueOf(length_S)); //2^g
        BigInteger right = S.add(BigInteger.ONE).mod(twoPowG); // MD5(S+1)mod2^g
        BigInteger MD4FromSplus1 = new BigInteger(getMD5Hash(right.toString()), 16);
    ...
    private void generatePandQ() throws NoSuchAlgorithmException {
        ...
        while(true) {
            ...
            for (int k = 0; k <= n; k++) {
                ...
                Vk[k] = new BigInteger(getMD5Hash(tmp.mod(tmp_pow).toString()), 16);
            }
    ...
    public PairForDSA doSignature(String fileName) throws NoSuchAlgorithmException, IOException {
        ...
        while (true) {
            ...
            hash_md5 = getMD5Hash(message);
            BigInteger hashMessage = new BigInteger(hash_md5, 16);
